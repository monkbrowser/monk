...
const eqSets = (setBefore, setAfter) => {
  for (const hn of setAfter) {
    if (setBefore.has(hn) === false) {
      return false;
    }
  }
  for (const hn of setBefore) {
    if (setAfter.has(hn) === false) {
      return false;
    }
  }
  return true;
...
async function filteringModesToDNR(modes) {
  ...
  if (eqSets(presentNone, modes.none)) {
    return;
  }
  const removeRuleIds = [];
  ...
  if (modes.none.size !== 0) {
    ...
    if (modes.none.size !== 1 || modes.none.has("all-urls") === false) {
...
export async function syncWithBrowserPermissions() {
  ...
  const allowedHostnames = new Set(
    hostnamesFromMatches(permissions.origins || [])
  );
  let modified = false;

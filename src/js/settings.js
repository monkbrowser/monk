...
function renderFilterLists(soft = false) {
  ...
  const listEntryCountFromGroup = function (groupRulesets) {
    if (Array.isArray(groupRulesets) === false) {
      return "";
    }
    let count = 0,
      total = 0;
  ...
  const liFromListGroup = function (groupKey, groupRulesets) {
    ...
    if (liGroup === null) {
      ...
      if (groupName === undefined) {
        groupName = i18n$(
          "3pGroup" + groupKey.charAt(0).toUpperCase() + groupKey.slice(1)
        );
  ...
  const groups = new Map([
    ["default", rulesetDetails.filter((ruleset) => ruleset.id === "default")],
    [
      "annoyances",
      rulesetDetails.filter((ruleset) => ruleset.group === "annoyances"),
    ],
    [
      "misc",
      rulesetDetails.filter(
        (ruleset) =>
          ruleset.id !== "default" &&
    ...
    [
      "regions",
      rulesetDetails.filter((ruleset) => typeof ruleset.lang === "string"),
    ],
  ]);
...
const renderWidgets = function () {
  ...
  qs$(
    `.filteringModeCard input[type="radio"][value="${defaultLevel}"]`
  ).checked = true;
  qs$('#autoReload input[type="checkbox"').checked =
    cachedRulesetData.autoReload;
...
function toggleHideUnusedLists(which) {
  ...
  for (const elem of qsa$(
    `#lists ${groupSelector} .listEntry[data-listkey] input[type="checkbox"]:not(:checked)`
  )) {
...
sendMessage({
  ...
  .then((data) => {
    ...
    rulesetMap.clear();
    cachedRulesetData.rulesetDetails.forEach((rule) =>
